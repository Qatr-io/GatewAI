...
- **Stages**: top-level `checks`/`action` = **input** (request). An optional nested `guardrails.output` block (`checks`/`action`) is the **output** stage — scans/redacts the model response (`choices[*].message.content`) in the LLM proxy (`internal/llmproxy/handler.go`). Streaming responses always degrade to `flag` (cannot redact/block mid-stream). An optional nested `guardrails.async` block (`checks`/`action`/`models`) is the **async result** stage — async job inputs are files, so the scannable content is the *result* (transcript, OCR text, ...). It runs in the gateway's once-per-job completion handler (`internal/handler/relaycomplete.go`, not the pub/sub broadcast → no per-replica double-count): fetch the result, run detectors over its text, and apply the stage action — `flag` (shadow, the safe default; observe only), `block` (fail the job and clear its result so the flagged content is never delivered — the client gets a failure webhook), or `redact` (regex only; write a redacted sibling object and repoint the job to it, retaining the original for audit). The **input** stage additionally guards text params on async submit (`POST /jobs`, e.g. a `prompt` field), scanned in `internal/handler/jobs.go` before enqueue; the uploaded file itself is not text-scanned. Metric `gatewai_guardrails_async_total{service_type, model, detector, result}` (`result` = `flagged`|`blocked`|`redacted`|`error`). Async result blocking fails open on a detector error (a transient model outage never fails a legitimate job).
