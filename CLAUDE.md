...
- **Stages**: top-level `checks`/`action` = **input** (request). An optional nested `guardrails.output` block (`checks`/`action`) is the **output** stage — scans/redacts the model response (`choices[*].message.content`) in the LLM proxy (`internal/llmproxy/handler.go`). Both stages also scan **tool-call arguments** (`messages[*].tool_calls[*].function.arguments` on the way in, `choices[*].message.tool_calls[*].function.arguments` on the way out), not just message content. **Streaming enforcement** (`guardrails.output.streaming`): `flag` (default) streams unbuffered and observes only; `block` and `buffer` hold a rolling window (`stream_window_tokens`, default 64 — the time-to-first-token vs enforcement trade-off) then, on the window boundary, either terminate the stream on a violation (`block`) or redact matched spans and release it (`buffer`) — see `internal/llmproxy/stream_guard.go`. Buffered modes always hold a trailing window so a match that straddles a release boundary is never split (correct for any match up to the window length; a pattern longer than the window may release a prefix before it completes — use a larger window or the non-streaming stage for long secrets). They emit synthesized `chat.completion.chunk` content deltas carrying the original `id`/`model`/`created`; control chunks (role/finish/usage/tool_calls) pass through, and a chunk with both content and `finish_reason` has its content buffered and its `finish_reason` forwarded. (Not yet covered: scanning **streamed** `delta.tool_calls` argument deltas — tool-call arg scanning applies to non-streaming and to the request side.)
