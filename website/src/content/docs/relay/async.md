...
The relay removes the job from `relay:<model>:processing` then exits `0`. KEDA creates a fresh pod for the next job.
...
| Relay pod crashes (no `Done`) | Job stays in `relay:<model>:processing`; recover manually with `LMOVE` (see [Runbooks](../manage/runbooks)) |
| Relay exits on BLMOVE timeout | Pod exits `0`, KEDA does not restart until queue is non-empty |

There are no automatic retries at the relay level. Resubmit failed jobs via `POST /jobs/{service_type}` if needed.
